<template>
  <div class="main-content">
        ...
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
		    ...
		    label="联系电话">
		     <template slot-scope="scope">
                ...
                <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                    prop="yuangongyouxiang"
                    header-align="center"
		    label="员工邮箱">
		     <template slot-scope="scope">
                       {{scope.row.yuangongyouxiang}}
                     </template>
                </el-table-column>
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('yuangong','绩效') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="jixiaokaoheCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'绩效':'' }}</el-button>
                <el-button v-if="isAuth('yuangong','绩效') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="jixiaokaoheCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'绩效':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('yuangong','绩效') && contents.tableBtnIcon == 0" type="success" size="mini" @click="jixiaokaoheCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'绩效':'' }}</el-button>
    ...
    <jixiaokaohe-cross-add-or-update v-if="jixiaokaoheCrossAddOrUpdateFlag" :parent="this" ref="jixiaokaoheCrossaddOrUpdate"></jixiaokaohe-cross-add-or-update>
...
import jixiaokaoheCrossAddOrUpdate from "../jixiaokaohe/add-or-update";
export default {
  data() {
    return {
      bumenOptions: [],
      ...
      jixiaokaoheCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    jixiaokaoheCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.jixiaokaoheCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','yuangong');
      this.$nextTick(() => {
      this.$refs.jixiaokaoheCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
          this.$http({
